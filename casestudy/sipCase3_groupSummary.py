...
data3cyl.set_index('vs', append = True).set_index('am', append = True).head() #multilevel indexing
...
#groupby : mean of milage grouped by gearx
...
dataGgcc.sort_index(level=[2,0])  #carb then gear
...
mtcars.sort_values(['mpg', 'hp']).head(5)
mtcars[['gear', 'cyl', 'wt']].sort_values(['gear', 'cyl', 'wt'], ascending = [True, False, True]).head(20)
...
data2.pivot_table(index=['gear', 'vs'], values='mpg', columns=['cyl', 'am'])
data2.pivot_table(index=['gear', 'vs'], columns=['cyl', 'am'], aggfunc = {'mpg' : np.mean, 'wt':min}, fill_value=0 )
data2.pivot_table(index=['gear'], columns=['vs', 'am'], aggfunc = {'mpg' : np.mean, 'wt':min}, fill_value=0 )
