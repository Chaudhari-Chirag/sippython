...
df2.groupby(['cyl','gear'], sort=True, as_index= True).aggregate({'mpg' :'mean'}) #with values sorted as per cyl and gear
...
#observed - uses only when tere is categorial value. 
df2['gear'] = df2['gear'].astype('category')
df2.groupby(['cyl','gear'], observed=True).aggregate({'mpg':'mean'}) #shows only observed values
df2.groupby(['cyl','gear'], observed=False).aggregate({'mpg':'mean'}) #shows all the valus or the category
pd.groupby?
