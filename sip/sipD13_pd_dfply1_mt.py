...
#%%
df1 >> select(X.mpg) #returns the column of mpg with carname as index used to select the column
...
#%%
#arrange
#three columns selected and then arranged in descending order of cly, am and mpg respectively.
...
df1 >> group_by(X.cyl) >> summarise(meanMPG = X.mpg.mean()) #grop by cly for mean of mpg
aa = df1 >> group_by(X.cyl,X.am) >> summarise(meanMPG = X.mpg.mean())
aa
aa.unstack()
#%%
